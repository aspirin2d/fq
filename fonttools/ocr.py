...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--input-font", required=True, help="Path to the input .woff2 font file."
    )
    parser.add_argument(
        "-o",
        "--output-dir",
...
def main():
    ...
    decompressed_ttf = os.path.join(
        args.output_dir, os.path.splitext(args.input_font)[0] + ".ttf"
    )
    ...
    for charcode, _ in char_map:
        # Step 4.1: Load & render the glyph into a grayscale bitmap
        face.load_char(chr(charcode), freetype.FT_LOAD_FLAGS.get("FT_LOAD_RENDER"))
        ...
        for line in ocr_result:
            ...
            if rect_texts:
                ...
                charcode_to_text[charcode] = ""
                print(f"{charcode} → Not Recognized")
    ...
    font_name = os.path.basename(args.input_font)
    updated_at = datetime.now().isoformat()

    codes_list = [
        {"key": str(code), "value": text} for code, text in charcode_to_text.items()
    ]

    output_obj = {"fontName": font_name, "updatedAt": updated_at, "codes": codes_list}
    ...
    print(f"\n[INFO] Mapping saved to: {mapping_path}")
